...
You are reading to know what you are looking at, not to produce one. **Read 3a-G's first four bullets, 3b and 3c — 30 lines, but long ones: about 700 words — and stop.** They say what a skeleton is, what the markers carry, and why each task looks the way it does. Everything else here is the generator's checklist, including the rest of 3a-G: "Guide mode and files that change" is about how to *write* a Before/After, not how to read one. So are the closure rules in 3d, the format shapes in 4b, and the Rules section. Skipping them costs you nothing, because the validators enforce them and the blueprint you are typing from has already passed.
...
**Scaffolding a feature that mostly changes existing files.** `guide scaffold` earns its keep when the feature creates files: it writes the skeletons so nobody transcribes forty of them by hand. A feature whose tasks are mostly `(modify)` creates almost nothing, so scaffolding writes almost nothing and the developer still types every hunk — and the files it does write land in a tree the hunks have not been applied to yet. Prefer plain `guide` there, and say so when you pick the mode.
...
- `**Test**`: optional, and only useful in a guide mode — the command that exercises behaviour, which
  ...
  **In `guide` mode, stamp a compile or syntax check, never a test run.** Guide skeletons are
  ...
  guide skeletons is red before the developer writes a line. Stamp `tsc --noEmit`, `javac`,
  `./gradlew compileKotlin` — a checker that reads the skeleton without running it. The full
  test command belongs in the full-code modes, where the bodies are real. Then read the next
  paragraph before you settle on a command: in a language whose only checker is a parser, this
  choice takes one more step.
...
- **A stand-in for an open question is marked everywhere it lands, tests included.** When Step 3d leaves a decision to the artifacts and the mode requires working code, the value you pick is a stand-in — say so in a comment beside it *and* beside every assertion that pins it. A test that fixes the boundary at 9000 with no note reads as a settled requirement to the next person, and the Open Questions row that says otherwise is a thousand lines away.
- **Identifiers carry their feature number outside their own document.** `OQ-1`, `D5`, `FR-004` and `T002` are unique inside one blueprint and collide across a repository: a single `cli.py` ends up carrying four features' `T002`. In code comments, in cleanup output and in anything a later reader greps, write `005 OQ-1`, `005 plan D5`. **This is checked**, inside code blocks — which is where the text becomes a comment in someone's tree. The round this rule was written, the next blueprint used `plan D…` 34 times and numbered none of them, and the developer typing it produced two `(plan D9)` comments in one file meaning different decisions of different features.
...
- **Implementation notes**: an ordered list of *what to achieve* in each body — behavior, edge cases, invariants to uphold, error handling — written as goals, never as line-by-line code dictation. Cite spec/plan/decision sections and official documentation URLs. **These are checked.** The validator reads implementation notes for pasteable expressions the same way it reads a marker's message; a note that is a Java statement with the punctuation removed is the body, wherever it is written. A reviewer's notes 2, 3 and 4 for one task were whole statements — and one of them inverted the sign of the arithmetic it dictated, which the developer only found by running the tests.
- **No body logic anywhere**: no branches, queries, transaction code, or assertion bodies — not in code blocks, not in a `// TODO(blueprint):` comment, not in the implementation notes, not spelled out in prose so literally that typing it is transcription. If a body is genuinely one obvious line (a delegation, a constant), say so in the notes instead of coding it. Shapes the validator reports: a boolean operator, a comparison with a call on one side, an arithmetic formula over identifiers (`spent/elapsed_days`, `avg*days_in_month`), a call carrying two or more keyword arguments, a subscripted call, an identifier compared against a number. Naming an API mid-sentence is what the mode asks for and passes.
...
- **One new file, several tasks.** The task that creates the file is `(new)` and carries the complete skeleton *as of that task*. Each later task that adds to it is `(modify)`: its Before quotes the smallest run of the previous skeleton that is unique in the file, and its After repeats that run with the new signature added. **Which run depends on the language.** Where a signature and its body are one line (`fun f() = TODO("…")`), the previous signature line alone is enough. Where the body is indented under the signature — Python, and any language whose skeleton spans lines — a signature line alone puts the new function between `def` and its body: quote the *next* declaration and add the new one above it, or quote the previous declaration through its closing line. Quoting the marker's message text works but breaks the moment that message is edited. The developer sees the file grow the way the tasks are ordered, and the applier can type it in that order.
- **Never replace working code with a marker.** A `(modify)` hunk whose Before holds lines that run and whose After puts a not-implemented marker where they were deletes shipped behavior. The build the header stamps compiles the skeleton and passes; the loss shows up in the project's own tests, which that build does not run. If a body genuinely has to be rewritten, say so in the implementation notes and leave the code in the After. This is a hard failure in the validator and has no judgment in it, so the check reads a marker call as the whole expression: a message that runs over several lines — Python implicit string concatenation, a Java text block, the shape a self-contained work instruction takes in any language — is the marker, not "working code" the next task is deleting. Quoting one task's multi-line marker in the next task's Before is a form the rules above ask for, and it used to fail this check.
- **Nested fences: four backticks outside.** A task whose quoted text is itself Markdown — a README update, which most features have — must wrap its `**Before**` and `**After**` blocks in ```` ```` ```` so the inner ``` ``` ``` does not close them. With three the quotation is silently cut at the first inner fence.
- **A change inside an existing body.** In guide mode there is usually no body to quote yet — it is the developer's to write — so the signature line is the only stable anchor: Before quotes the signature (and its marker line where one exists), After repeats it with the marker comment added above or beside it. The comment is not a place to hide the body: the same dictation check runs over `// TODO(blueprint):` text as over a marker's message. Where a body does exist (a `(modify)` task against already-written code), Before quotes the lines around the spot; After is the same lines with one marker comment inserted where the change goes — `// TODO(blueprint): T0NN: <what to change and why>` in the file's comment syntax — and the implementation notes say what the change must achieve. Cleanup recognises that marker and sweeps it once the change is made.
...
- **Every symbol you name must resolve where you name it.** *(every project — a single-package script still has a standard library)* For each type or function the instructions tell the developer to catch, throw, call, or return, give the fully-qualified name and the module that supplies it, then check it against the target module's declared dependencies. **And against the skeleton's own import list.** A marker that says to use `dataclasses.replace` in a file whose skeleton never imports `replace` parses, passes a syntax-only build, and dies with a `NameError` the first time the developer runs it — the import is part of the work being handed over, not something the reader is expected to infer. If it is not on that module's classpath, the blueprint must include the task that adds the dependency — with whatever justification the project's rules require. Deferring a type to a later test ("the exact exception is confirmed by T0NN") is allowed only if every task that needs the type is marked as blocked on that test.
