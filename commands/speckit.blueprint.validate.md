...
If `check-prerequisites.sh` is missing or reports `Feature directory not found`, do not stop: none of the three scripts need it. Each resolves the feature from the branch prefix, or from a path given as the first argument, so `specs/NNN-name` on the command line is enough.
...
- `--verify` — after a clean apply, run each applied task's `**Verification**` command in the copy. A backticked span that begins with a runner (`python3`, `bash`, `node`, `mvn`, `go`, `./gradlew`, …) is a command; everything else in the line is prose and is left alone. A task naming no runnable command is counted, not failed. Any verification that exits non-zero fails the run
